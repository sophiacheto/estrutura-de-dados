...
//  -----------------------
   ...
   public int findDepth(BTNode<T> n, int x) {
    // Base
    if (n == null) return -1;
 
    int dist = -1;
    if ((root.getValue().equals(x)) || (dist = findDepth(n.getLeft(), x)) >= 0 || (dist = findDepth(n.getRight(), x)) >= 0)
      return dist + 1;
